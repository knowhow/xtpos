...
BEGIN  
  ...
  FOR _sale IN
    SELECT
      ...
      saleitem_taxtype_id, saleitem_tax_id, 
      saleitem_tax_pcta, saleitem_tax_pctb, saleitem_tax_pctc,
      saleitem_tax_ratea, saleitem_tax_rateb, saleitem_tax_ratec,
  ...
  LOOP
	...
	WHERE (salesaccnt_id=_sale.sales_accnt_id);
      ...
      -- Credit tax accounts
      _taxBaseValue := addTaxToGLSeries(_seqRev, basecurrid(),
	   			         'S/O', 'RS', _rec.terminal_number,
				         current_date, current_date,
				         _sale.saleitem_tax_id,
				         _sale.saleitem_tax_ratea,
				         _sale.saleitem_tax_rateb,
				         _sale.saleitem_tax_ratec, _noteSales);
      IF (_taxBaseValue IS NULL) THEN
        RAISE EXCEPTION 'Error processing taxes';
      END IF;

      -- Record sales history
      INSERT INTO cohist
      ( cohist_cust_id, cohist_itemsite_id, cohist_tax_id,
      ...
      VALUES
      ( _sale.salehead_cust_id, _sale.saleitem_itemsite_id, _sale.saleitem_tax_id,
        ...
        _sale.saleitem_tax_pcta, _sale.saleitem_tax_pctb, _sale.saleitem_tax_pctc,
        COALESCE(_sale.saleitem_tax_ratea,0.0), COALESCE(_sale.saleitem_tax_rateb,0.0), COALESCE(_sale.saleitem_tax_ratec,0.0));
    END IF;
  END LOOP;
