...
_creditGroup.visible = (_ccp != null && _ccp.testConfiguration() >= 0);
if (! _creditGroup.visible)
{
  try {
    if (_ccp)
      print(_ccp.errorMsg());
...
function calc()
{
  ...
  if (_cashAmount.localValue >= _total.localValue)
  {
    ...
    _creditGroup.setChecked(0);
    _creditGroup.setEnabled(0);
    _creditAmount.localValue = 0;
    _checkGroup.setChecked(0);
    _checkGroup.setEnabled(0); 
    _checkAmount.localValue = 0;
  ...
  {
    _creditGroup.setEnabled(1);
    if (_type != "Return")
      _checkGroup.setEnabled(1); 
  }
}
...
try {
  ...
  print("payment::set() exception at " + e.lineNumber + ": " + e);
}
}
